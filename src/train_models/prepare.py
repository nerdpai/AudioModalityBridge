...
def prepare_bridge_params(model: Union[DataParallel[VoiceLM], VoiceLM]):
    model = get_model(model)
...
def prepare_audio_model_params(model: Union[DataParallel[VoiceLM], VoiceLM]):
    model = get_model(model)
    ...
    return model.audio_bridge.audio_model.parameters()
